defmodule ElixirMostWanted.Github do
  def authorize_url(return_to \\ nil) do
    ...
    query =
      URI.encode_query(
        client_id: client_id(),
        state: ElixirMostWanted.Util.random_string(),
        scope: "user:email",
        redirect_uri: "#{ElixirMostWantedWeb.Endpoint.url()}/oauth/callbacks/github?#{redirect_query}"
      )
  ...
  defp client_id, do: ElixirMostWanted.config([:github, :client_id])
  defp secret, do: ElixirMostWanted.config([:github, :client_secret])
