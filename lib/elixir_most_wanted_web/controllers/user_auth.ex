defmodule ElixirMostWantedWeb.UserAuth do
  use ElixirMostWantedWeb, :verified_routes
  import Plug.Conn
    ...
    case session do
      %{"user_id" => user_id} ->
        ...
        if Accounts.admin?(user) do
          ...
          {:halt, LiveView.redirect(socket, to: ~p"/status/404")} # TODO: implement this route
        end
  ...
  def log_out_user(conn) do
    if live_socket_id = get_session(conn, :live_socket_id) do
      ElixirMostWantedWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def require_authenticated_admin(conn, _opts) do
    ...
    if user && ElixirMostWanted.Accounts.admin?(user) do
